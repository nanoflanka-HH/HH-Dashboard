exports.handler = async function(event) {
  ...
  try {
    ...
    const noteKey = String(body.noteKey || (section === 'quarterly' ? 'quarterly' : 'general'));
    const folderMap = {
    ...
    if (!owner || !repo || !secret) throw new Error('Missing GitHub environment variables.');
    ...
    const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(path).replace(/%2F/g, '/')}`, {
      ...
      body: JSON.stringify({ message: `save ${section} ${noteKey} note ${timestamp}`, content, branch })
    });

    if (!res.ok) throw new Error(await res.text());
    return { statusCode: 200, headers: { ...cors, 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: true, path }) };
  } catch (err) {
